...
Each generated problem is reviewed by a critic before it may enter a pack. The critic family is
...
* the **deterministic linter** (step 2), the only step identical on every validator, which rejects
  on structure rather than judgement;
* the **discrimination probe** (step 5), the strongest filter in the pipeline. A vague problem
  fails it on condition 3 — judges scoring the same portfolio far apart across repeats — which is
  a measurement of vagueness rather than an opinion about it.

The critic is therefore the weakest of three filters rather than one of two. It is retained
because it is one model call against the probe's four laboratory runs, and it rejects candidates
the linter cannot see.

The two-generator split of 7.2.1 is unchanged: ten problems from each family, so half the pack is
always foreign to whatever a laboratory has overfitted to, and the per-family score gap remains
the signal that the challenge supply has become predictable.
